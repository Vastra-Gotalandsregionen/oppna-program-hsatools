...
function initAutocompleter() {
	...
	var itemSelectHandler = function(sType, aArgs) {
		...
		var hsaId = oData.id;
		if (hsaId != '') {
			document.location = "http://hittavard.vgregion.se/hriv/visaenhet?hsaidentity=" + hsaId;
		}
	};
	...
	var readyHandler = function(oSelf) {
		...
		document.getElementById('autocomplete_outer').style.display = 'block';
		document.getElementById('healthcare-selection').style.padding = "2em 0 0 0";
	};

	var doneHandler = function(oSelf) {
		document.getElementById('autocomplete_outer').style.display = 'none';
	};
	...
	myAutoComp.containerCollapseEvent.subscribe(doneHandler);
}
