...
public class UnitRepositoryKivws implements UnitRepository {
	private static final DistinguishedName KIV_SEARCH_BASE = new DistinguishedName(
			"ou=Org,o=vgr");
	...
	public UnitRepositoryKivws(
			VGRegionWebServiceImplPortType vgregionWebService,
			KivwsUnitMapper kivwsUnitMapper, CodeTablesService codeTablesService) {
	 ...
	 *            - unit to search for.
	 * @param maxResult
	 *            - max result of found units to return.
	 * @param sortOrder
	 *            - sort order for the result list.
	 * @param onlyPublicUnits
	 *            - show units that should be displayed to the public.
	 * 
	...
	public SikSearchResultList<Unit> searchAdvancedUnits(Unit unit,
			...
			throws KivException {
		String searchFilterOU = this.createAdvancedSearchFilter(unit,
				onlyPublicUnits, false);
		String searchFilterCN = this.createAdvancedSearchFilter(unit,
				onlyPublicUnits, true);

		List<Unit> searchOUresult = this
				.searchUnits(this.getSearchBase(), searchFilterOU,
						SearchControls.SUBTREE_SCOPE, ATTRIBUTES, false);
		List<Unit> searchCNResult = this.searchFunctionUnits(
				this.getSearchBase(), searchFilterCN,
				SearchControls.SUBTREE_SCOPE, ATTRIBUTES);
		searchOUresult.addAll(searchCNResult);
		SikSearchResultList<Unit> units = this.cleanAndSortResult(
				searchOUresult, sortOrder);
	...
	protected void removeUnallowedUnits(SikSearchResultList<Unit> units) {
		...
		List<HealthcareType> allUnfilteredHealthcareTypes = htch
				.getAllUnfilteredHealthCareTypes();

		for (int j = units.size() - 1; j >= 0; j--) {
			if (!this.unitMatchesUnfilteredHealtcareType(units.get(j),
					allUnfilteredHealthcareTypes)) {
				units.remove(units.get(j));
			}
		}
	}

	private boolean unitMatchesUnfilteredHealtcareType(Unit unit,
			List<HealthcareType> allUnfilteredHealthcareTypes) {
		boolean found = false;
		for (HealthcareType h : allUnfilteredHealthcareTypes) {
			for (Map.Entry<String, String> condition : h.getConditions()
					.entrySet()) {
	...
	public SikSearchResultList<Unit> searchUnits(
			...
			throws KivException {
		String searchFilterOU = this.createSearchFilter(searchUnitCriterions,
				false);
		String searchFilterCN = this.createSearchFilter(searchUnitCriterions,
				true);

		List<Unit> searchResultForOuUnits = this.searchUnits(
				this.getSearchBase(), searchFilterOU,
				SearchControls.SUBTREE_SCOPE, ATTRIBUTES, false);
		List<Unit> searchResultForCnUnits = this.searchFunctionUnits(
				this.getSearchBase(), searchFilterCN,
				SearchControls.SUBTREE_SCOPE, ATTRIBUTES);
		searchResultForOuUnits.addAll(searchResultForCnUnits);

		SikSearchResultList<Unit> result = this.cleanAndSortResult(
				searchResultForOuUnits, new UnitNameComparator());
		// Make sure that only a subset is returned
		return this.getMaxResultList(maxResult, result);
	}

	private SikSearchResultList<Unit> getMaxResultList(int maxResult,
	...
	public Unit getUnitByHsaId(String hsaId) throws KivException {
		return getUnitByHsaId(hsaId, true);
	}

	private Unit getUnitByHsaId(String hsaId, boolean mapDeliveryPoint)
			throws KivException {
		Unit pUnit;
		String searchFilter = "(hsaIdentity=" + hsaId + ")";
		pUnit = this.searchUnit(this.getSearchBase(),
				SearchControls.SUBTREE_SCOPE, searchFilter, mapDeliveryPoint);
		if (pUnit.getHsaHealthCareUnitManagerHsaId().length() > 0
				&& pUnit.getHsaHealthCareUnitManagerHsaId() != null) {
			pUnit.setHsaHealthCareUnitManagerPerson(this
					.getHsaHealthCareUnitManagerByHsaID(pUnit
							.getHsaHealthCareUnitManagerHsaId()));
		}
		if (pUnit.getHsaHealthCareUnitMembers().size() > 0) {
			pUnit.setHsaHealthCareUnitMembersAsUnit(this
					.getSimpleUnitsByHsaId(pUnit.getHsaHealthCareUnitMembers()));
		}
	...
	/**
	 * Fetch unit by the unit hsa id without deliverypoint
	 * 
	 * @param hsaId
	 *            The hsa id of the unit.
	 * @return The unit with the given hsa id.
	...
	public Unit getUnitByHsaIdtWihoutDeliverypoints(String hsaId)
			throws KivException {
		return getUnitByHsaId(hsaId, false);
	}
	 ...
	 * 
	 */
	public String getHsaIdWhereUnitIsHealthCareMember(String hsaId)
			throws KivException {
		Unit pUnit;
		String pRetValue = "";
		String searchFilter = "(hsaHealthCareUnitMember=" + hsaId + ")";

		pUnit = this.searchUnitWithoutRoles(this.getSearchBase(),
	...
	/**
	 * 
	 *  Set vgrOrgRel units on a persons employment. Used to pressent phonenumbers on user. 
	 *  
	 *  @param SikSearchResultList<Person> persons
	 *	@return SikSearchResultList<Person> persons
	 */
	@Override
	public SikSearchResultList<Person> setUnitOnEmployments(
			SikSearchResultList<Person> persons) {
		List<String> orFilterList = new ArrayList<String>();

		for (Person person : persons) {
			for (String vgrOrgRel : person.getVgrOrgRel()) {
				orFilterList.add("(hsaIdentity=" + vgrOrgRel + ")");
			}
		}

		List<Unit> pUnits = this.searchUnits(this.getSearchBase(),
				this.makeOr(orFilterList), SearchControls.SUBTREE_SCOPE,
				ATTRIBUTES, false);

		Hashtable<String, Unit> pTableOfUnits = new Hashtable<String, Unit>();
		for (Unit unit : pUnits) {
			pTableOfUnits.put(unit.getHsaIdentity(), unit);
		}

		for (Person person : persons) {
			if (person.getEmployments() != null) {
				for (Employment employment : person.getEmployments()) {
					if (pTableOfUnits.containsKey(employment.getVgrOrgRel())
							&& employment.getVgrOrgRel() != "") {
						employment.setConnectedUnit(pTableOfUnits
								.get(employment.getVgrOrgRel()));
					}
				}
			}
		}
	...
	/**
	 * Method to get all related helthCareUnitMembers. When using
	 * "getunitByHsaId" the search sometimes took over 60 seconds! Only used in
	 * setHsaHealthCareUnitMembersAsUnit.
	 * 
	 * @param hsaIds
	 *            , List<String>
	 ...
	 * @throws KivException
	 */
	public List<Unit> getSimpleUnitsByHsaId(List<String> hsaIds)
			throws KivException {
		Unit pUnit = null;
		...
		try {
			// Get units
			pAttributes = this.vgregionWebService
					.getReturnAttributesForUnit(VGRegionDirectory.KIV);
			ArrayOfUnit searchUnits = this.vgregionWebService.searchUnit(
					pSearchFilter, pAttributes, VGRegionDirectory.KIV,
					"ou=org,o=vgr", "2");

			List<Entry> attributes;
			for (se.vgregion.kivtools.search.svc.ws.domain.kivws.Unit u : searchUnits
					.getUnit()) {
				Unit unit = new Unit();
				...
				for (Entry entry : attributes) {
					if (entry.getKey().equals("hsaidentity")) {
						...
						unit.setHsaIdentity(entry.getValue().getAnyType()
								.get(0).toString());
					} else if (entry.getKey().equals("ou")) {
						// Is single value attribute, safe to use .get index 0.
						unit.setName(entry.getValue().getAnyType().get(0)
								.toString());
					}
				}
			...
			// Get functions
			pAttributes = this.vgregionWebService
					.getReturnAttributesForFunction(VGRegionDirectory.KIV);
			ArrayOfFunction searchFunctions = this.vgregionWebService
					.searchFunction(pSearchFilter, pAttributes,
							VGRegionDirectory.KIV, "ou=org,o=vgr", "2");
			for (se.vgregion.kivtools.search.svc.ws.domain.kivws.Function u : searchFunctions
					.getFunction()) {
				Unit unit = new Unit();
				attributes = u.getAttributes().getValue().getEntry();
				for (Entry entry : attributes) {
				...
				unit.setName(u.getDn().getValue()
						.substring(3, u.getDn().getValue().indexOf(",")));
				unit.setDn(DN.createDNFromString(u.getDn().getValue()).escape());
				pReturnValues.add(unit);
			}
	...
	private Person getHsaHealthCareUnitManagerByHsaID(
			String hsaHealthCareUnitManagerHsaId) {
		...
		Person pPerson = null;
		try {
			pAttributes = vgregionWebService
					.getReturnAttributesForPerson(VGRegionDirectory.KIV);
			pPersons = this.vgregionWebService.searchPerson(pSearchFilter,
					pAttributes, VGRegionDirectory.KIV, "o=vgr", "2")
					.getPerson();
	...
	public Unit getUnitByHsaIdAndHasNotCareTypeInpatient(String hsaId)
		...
		Filter careTypesFilterList = this.generateCareTypeFilterFromList(
				KivwsCodeTableName.VGR_CARE_TYPE,
				UnitSearchAttributes.CARE_TYPE, careTypes);
		...
		// return this.searchUnit(this.getSearchBase(),
		// SearchControls.SUBTREE_SCOPE, searchFilterString);

		Unit pUnit = this.searchUnit(this.getSearchBase(),
				SearchControls.SUBTREE_SCOPE, searchFilterString, true);
		if (pUnit.getHsaHealthCareUnitManagerHsaId().length() > 0
				&& pUnit.getHsaHealthCareUnitManagerHsaId() != null) {
			pUnit.setHsaHealthCareUnitManagerPerson(this
					.getHsaHealthCareUnitManagerByHsaID(pUnit
	...
	public Unit getUnitByDN(DN dn) throws KivException {
		Unit u = null;
		DistinguishedName distinguishedName = new DistinguishedName(dn.escape()
				.toString());
		u = this.lookupUnit(distinguishedName, ATTRIBUTES);

		if (u.getHsaHealthCareUnitManagerHsaId().length() > 0
				&& u.getHsaHealthCareUnitManagerHsaId() != null) {
			u.setHsaHealthCareUnitManagerPerson(this
					.getHsaHealthCareUnitManagerByHsaID(u
							.getHsaHealthCareUnitManagerHsaId()));
		}
		if (u.getHsaHealthCareUnitMembers().size() > 0) {
			u.setHsaHealthCareUnitMembersAsUnit(this.getSimpleUnitsByHsaId(u
					.getHsaHealthCareUnitMembers()));
		}
		u.setHsaHealthCareMemberOf(getHsaIdWhereUnitIsHealthCareMember(u
				.getHsaIdentity()));
	...
	public List<String> getAllUnitsHsaIdentity(boolean onlyPublicUnits)
		...
		// Since SingleAttributeMapper return a String we are certain that the
		// cast to List<String> is ok
		List<String> result = this.searchSingleAttribute(this.getSearchBase(),
				searchFilter, SearchControls.SUBTREE_SCOPE,
				Arrays.asList("hsaIdentity"), UnitLdapAttributes.HSA_IDENTITY);
		return result;
	}
	...
	public List<Unit> getAllUnits(boolean onlyPublicUnits) {
		...
		result.addAll(this.searchUnits(this.getSearchBase(), searchFilter,
				SearchControls.SUBTREE_SCOPE, ATTRIBUTES, false));
		result.addAll(this.searchFunctionUnits(this.getSearchBase(),
				searchFilter, SearchControls.SUBTREE_SCOPE, ATTRIBUTES));
		return result;
	}
	...
	private Unit searchUnit(DistinguishedName searchBase, int searchScope,
			String searchFilter, boolean mapDeliverypoint) throws KivException {
		// Since UnitMapper return Units we are certain that the suppression is
		// ok
		List<Unit> result = this.searchUnits(searchBase, searchFilter,
				searchScope, ATTRIBUTES, mapDeliverypoint);
		List<Unit> resultFunctions = this.searchFunctionUnits(searchBase,
				searchFilter, searchScope, ATTRIBUTES);
	 ...
	 * @throws KivException
	 */
	private Unit searchUnitWithoutRoles(DistinguishedName searchBase,
			int searchScope, String searchFilter) throws KivException {
		// Since UnitMapper return Units we are certain that the suppression is
		// ok
		Unit pRetUnit;
		List<Unit> result = this.searchUnits(searchBase, searchFilter,
				searchScope, ATTRIBUTES, false);
		if (result.size() == 0) {
			pRetUnit = null;
		} else {
	...
	private SikSearchResultList<Unit> cleanAndSortResult(List<Unit> units,
			Comparator<Unit> sortOrder) {
	...
	private SikSearchResultList<Unit> deduplicateResult(
			SikSearchResultList<Unit> result) {
	 ...
	 */
	private String makeFunctionSearchStringFromUnitSearchString(
			String unitSearchString) {
		String functionSearchString = Formatter.replaceStringInString(
				unitSearchString, Constants.OBJECT_CLASS_UNIT_SPECIFIC,
				Constants.OBJECT_CLASS_FUNCTION_SPECIFIC);
		functionSearchString = Formatter.replaceStringInString(
				functionSearchString, Constants.OBJECT_CLASS_UNIT_STANDARD,
				Constants.OBJECT_CLASS_FUNCTION_STANDARD);
		functionSearchString = Formatter.replaceStringInString(
				functionSearchString, Constants.LDAP_PROPERTY_UNIT_NAME + "=",
	 ...
	 * @throws KivException
	 */
	String createSearchFilter(SearchUnitCriterions searchUnitCriterions,
			boolean createCNSearchString) throws KivException {
		...
		if (createCNSearchString) {
			...
			String functionSearchString = this
					.makeFunctionSearchStringFromUnitSearchString(result);
	...
	String createAdvancedSearchFilter(Unit unit, boolean onlyPublicUnits,
			boolean createFunctionSearchFilter) {
		// create a plain unit search filter
	...
	protected String createUnitSearchFilter(
			SearchUnitCriterions searchUnitCriterions) {
		List<String> filterList = new ArrayList<String>();
		...
		if (!StringUtil.isEmpty(searchUnitCriterions.getAdministrationName())) {
			Filter orFilter = this.generateOrFilterFromList(
					KivwsCodeTableName.VGR_AO3_CODE,
					UnitSearchAttributes.ADMINISTRATION,
					searchUnitCriterions.getAdministrationName());
			andFilter.and(orFilter);
		}
		if (!StringUtil.isEmpty(searchUnitCriterions.getLiableCode())) {
			andFilter.and(this.createSearchFilter("vgrAnsvarsnummer",
					searchUnitCriterions.getLiableCode()));
		}
		if (!StringUtil.isEmpty(searchUnitCriterions
				.getBusinessClassificationName())) {
			Filter orFilter = this.generateOrFilterFromList(
					KivwsCodeTableName.HSA_BUSINESSCLASSIFICATION_CODE,
					UnitSearchAttributes.BUSINESS_CLASSIFICATION_CODE,
		...
		if (!StringUtil.isEmpty(searchUnitCriterions.getCareTypeName())) {
			Filter orFilter = this.generateOrFilterFromList(
					KivwsCodeTableName.VGR_CARE_TYPE,
					UnitSearchAttributes.CARE_TYPE,
					searchUnitCriterions.getCareTypeName());
			andFilter.and(orFilter);
		}
		// create or criteria
		if (!StringUtil.isEmpty(searchUnitCriterions.getLocation())) {
			...
			// orMunicipalityName.or(this.createSearchFilter("hsaMunicipalityName",
			// LdapParse.escapeLDAPSearchFilter(searchUnitCriterions.getLocation())));
			// Create or criteria a bit special...
			OrFilter orPostalAddress = this.createAddressSearchFilter(
					filterList, "hsaPostalAddress", LdapParse
							.escapeLDAPSearchFilter(searchUnitCriterions
									.getLocation()));
			// Create or criteria
			OrFilter orStreetAddress = this.createAddressSearchFilter(
					filterList, "hsaStreetAddress", LdapParse
							.escapeLDAPSearchFilter(searchUnitCriterions
									.getLocation()));
	...
	private Filter generateOrFilterFromList(KivwsCodeTableName codeTableName,
			UnitSearchAttributes criterion, String criterionValue) {
		List<String> codeFromTextValues = this.codeTablesService
				.getCodeFromTextValue(codeTableName, criterionValue);
		...
		if (codeFromTextValues.size() > 0) {
			...
			orFilter.or(new EqualsFilter(criterion.toString(),
					"NO_VALID_CODE_TABLE_CODE_FOUND"));
		}
		return orFilter;
	}

	private Filter generateCareTypeFilterFromList(
			KivwsCodeTableName codeTableName, UnitSearchAttributes criterion,
			List<String> careTypes) {
		List<String> codeFromTextValues = new ArrayList<String>();
		for (String careType : careTypes) {
			codeFromTextValues.addAll(this.codeTablesService
					.getCodeFromTextValue(codeTableName, careType));
		}
	...
	protected String createAdvancedUnitSearchFilter(Unit unit) {
		...
		// or criterias
		this.addSearchFilter(filterList, "hsaMunicipalityName",
				LdapParse.escapeLDAPSearchFilter(unit.getHsaMunicipalityName()));
		if (unit.getHsaMunicipalityCode() != null) {
			this.addSearchFilter(filterList, "hsaMunicipalityCode", LdapParse
					.escapeLDAPSearchFilter(unit.getHsaMunicipalityCode()));
		}

		// a bit special...
		this.createAddressSearchFilter(filterList, "hsaPostalAddress",
				LdapParse.escapeLDAPSearchFilter(unit.getHsaMunicipalityName()));

		this.createAddressSearchFilter(filterList, "hsaStreetAddress",
				LdapParse.escapeLDAPSearchFilter(unit.getHsaMunicipalityName()));
		// (|(par1=value1)(par2=value2))
		String orCriterias = this.makeOr(filterList);

		filterList = new ArrayList<String>();
		this.addSearchFilter(filterList, Constants.LDAP_PROPERTY_UNIT_NAME,
				unit.getName());
		if (!StringUtil.isEmpty(unit.getName())) {
			List<String> matchingHsaBusinessClassificationCodes = this.codeTablesService
					.getCodeFromTextValue(
							KivwsCodeTableName.HSA_BUSINESSCLASSIFICATION_CODE,
							unit.getName());
			for (String matchingHsaBusinessClassificationCode : matchingHsaBusinessClassificationCodes) {
				this.addSearchFilter(filterList,
						"hsaBusinessClassificationCode",
						matchingHsaBusinessClassificationCode);
			}
		}
		...
		if (unit.getHealthcareTypes() != null
			...
			this.addHealthCareTypeConditions(filterList,
					unit.getHealthcareTypes());
		}
	 ...
	 * result=(givenName=hans-erik)
	 */
	private void addSearchFilter(List<String> filterList, String searchField,
			String searchValue) {
		...
		if (!StringUtil.isEmpty(searchValueCurrent)) {
			...
			if (this.isExactMatchFilter(searchValueCurrent)) {
				...
				// exact match
				filterList.add("(" + searchField + "="
						+ searchValueCurrent.trim() + ")");
			} else {
	...
	private Filter createSearchFilter(String searchField, String searchValue) {
		...
		if (!StringUtil.isEmpty(currentSearchValue)) {
			...
			if (this.isExactMatchFilter(currentSearchValue)) {
				...
				currentSearchValue = Formatter.replaceStringInString(
						currentSearchValue, "-", LDAP_WILD_CARD);
				filter = new LikeFilter(searchField, LDAP_WILD_CARD
						+ currentSearchValue + LDAP_WILD_CARD);
			}
		}
		return filter;
	 ...
	 * =*$*$*$*$*uddevalla*$*)(hsaPostalAddress=*$*$*$*$*$*uddevalla*))
	 */
	private OrFilter createAddressSearchFilter(List<String> filterList,
	 ...
	 */
	private void addHealthCareTypeConditions(List<String> filterList,
			List<HealthcareType> allUnfilteredHealthcareTypes) {
		for (HealthcareType h : allUnfilteredHealthcareTypes) {
			...
			for (Map.Entry<String, String> condition : h.getConditions()
				...
				for (int i = 0; i < conditionValues.length; i++) {
					conditionComponentFilter.add("(" + condition.getKey() + "="
							+ conditionValues[i] + ")");
				}
				String conditionComponentsQuery = this
						.makeOr(conditionComponentFilter);
	 ...
	 * 
	 * @param parentUnit
	 *            - unit to get subunits for
	 * @param maxResult
	 *            - maximum of unit to be return in the result
	 * @return A list of subunits for current unit.
	 * @throws KivException
	 *             If something goes wrong doing search.
	 */
	@Override
	public SikSearchResultList<Unit> getSubUnits(Unit parentUnit, int maxResult)
			throws KivException {
		SikSearchResultList<Unit> subUnits = null;

		DistinguishedName parentDn = new DistinguishedName(parentUnit.getDn()
				.toString());

		// Since UnitMapper return a Unit we are certain that the cast to
		// List<Unit> is ok
		List<Unit> searchOU = this.searchUnits(parentDn, "(ou=*)",
				SearchControls.SUBTREE_SCOPE, ATTRIBUTES, false);
		List<Unit> searchCN = this.searchFunctionUnits(parentDn, "(cn=*)",
				SearchControls.SUBTREE_SCOPE, ATTRIBUTES);
	...
	public SikSearchResultList<Unit> getFirstLevelSubUnits(Unit parentUnit)
		...
		// Since UnitMapper return a Unit we are certain that the cast to
		// List<Unit> is ok
		List<Unit> search = this.searchUnits(parentDn, "(ou=*)",
				SearchControls.ONELEVEL_SCOPE, ATTRIBUTES, false);
	...
	private void removeUnitParentFromList(Unit parentUnit,
			SikSearchResultList<Unit> subUnits) {
	...
	private Unit lookupUnit(Name name, List<String> attrs) {
		...
		try {
			...
			ArrayOfUnit searchUnit = this.vgregionWebService.searchUnit(filter,
					arrayOfString, VGRegionDirectory.KIV, base.toString(),
					Integer.toString(SearchControls.ONELEVEL_SCOPE));
	...
	private List<Unit> searchFunctionUnits(Name base, String filter,
			int searchScope, List<String> attrs) {
		List<Unit> resultUnits = new ArrayList<Unit>();
		ArrayOfString arrayOfString = new ArrayOfString();
		arrayOfString.getString().addAll(attrs);
		try {
			ArrayOfFunction searchFunction = this.vgregionWebService
					.searchFunction(
							StringUtils.defaultIfEmpty(filter, "(cn=*)"),
							arrayOfString, VGRegionDirectory.KIV,
							base.toString(), Integer.toString(searchScope));
	...
	private List<String> searchSingleAttribute(Name base, String filter,
			int searchScope, List<String> attrs, String mappingAttribute) {
		List<String> result = new ArrayList<String>();
		SingleAttributeMapper singleAttributeMaper = new SingleAttributeMapper(
				mappingAttribute);
		ArrayOfString arrayOfString = new ArrayOfString();
		arrayOfString.getString().addAll(attrs);
		try {
			...
			ArrayOfFunction searchFunction = this.vgregionWebService
					.searchFunction(filter, arrayOfString,
							VGRegionDirectory.KIV, base.toString(),
							Integer.toString(searchScope));
			ArrayOfUnit searchUnits = this.vgregionWebService.searchUnit(
					filter, arrayOfString, VGRegionDirectory.KIV,
					base.toString(), Integer.toString(searchScope));
			List<Function> functions = searchFunction.getFunction();
			List<se.vgregion.kivtools.search.svc.ws.domain.kivws.Unit> units = searchUnits
					.getUnit();
	...
	private List<Unit> searchUnits(Name base, String filter, int searchScope,
			List<String> attrs, boolean mapDeliverypoints) {
		List<Unit> result = new ArrayList<Unit>();
		ArrayOfString arrayOfString = new ArrayOfString();
		arrayOfString.getString().addAll(attrs);
		...
		try {
			...
			ArrayOfUnit searchUnit = this.vgregionWebService.searchUnit(
					StringUtils.defaultIfEmpty(filter, "(ou=*)"),
					arrayOfString, VGRegionDirectory.KIV, base.toString(),
					Integer.toString(searchScope));
			//long stop = System.currentTimeMillis();

			//System.out.println("Search unit on WS: " + (stop - start));

			//stop = System.currentTimeMillis();
			result = this.mapKivwsUnits(searchUnit, mapDeliverypoints);
			//stop = System.currentTimeMillis();
			//System.out.println("Unit mapping: " + (stop - start));
	...
	private List<Unit> mapKivwsUnitToUnit(ArrayOfUnit arrayOfUnit,
			boolean mapDeliverypoint) {
		List<se.vgregion.kivtools.search.svc.ws.domain.kivws.Unit> unit = arrayOfUnit
				.getUnit();
		List<Unit> result = new ArrayList<Unit>();
	...
	private List<Unit> mapKivwsUnitFunctionToUnit(
			ArrayOfFunction arrayOfFunction, boolean mapDeliverypoint) {
		List<Unit> result = new ArrayList<Unit>();
