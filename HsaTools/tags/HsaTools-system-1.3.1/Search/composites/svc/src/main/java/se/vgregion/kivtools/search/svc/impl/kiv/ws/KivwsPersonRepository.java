...
public class KivwsPersonRepository {
  ...
  private String createSearchPersonsFilterVgrId(String vgrId) throws KivException {
    return this.createSearchFilterItem("vgr-id", vgrId);
  }
  ...
  public SikSearchResultList<Person> getPersonsForUnits(List<Unit> units, int maxResult) throws KivException {
    ...
    persons = this.searchPersons(unitFkfilter.encode(), SearchControls.SUBTREE_SCOPE, maxResult);
    return persons;
  }
  ...
  private AndFilter generateFreeTextSearchPersonFilter(SearchPersonCriterions person) {
    ...
    if (!StringUtil.isEmpty(person.getGivenName())) {
      ...
      firstNameFilter.or(new LikeFilter(PersonSearchAttributes.GIVEN_NAME.toString(), "*" + person.getGivenName() + "*"));
    ...
    if (!StringUtil.isEmpty(person.getSurname())) {
      ...
      surnameFilter.or(new LikeFilter(PersonSearchAttributes.SURNAME.toString(), "*" + person.getSurname() + "*"));
