...
public final class DN implements Serializable, Iterable<DN> {
  ...
  public DN(List<String> cn, List<String> ou, List<String> dc, String o) {
    this.cn = this.defensiveCopy(cn);
    this.ou = this.defensiveCopy(ou);
    this.dc = this.defensiveCopy(dc);
  ...
  private DN(DN originalDn, int position) {
    this.cn = this.defensiveCopy(originalDn.cn);
    this.ou = this.defensiveCopy(originalDn.ou);
    this.dc = this.defensiveCopy(originalDn.dc);
    this.o = originalDn.o;
  ...
  /**
   * Escapes any comma and plus in the DN's common names, organizational units and domain components.
   * 
   * @return A new DN with commas and pluses escaped.
   */
  public DN escape() {
    ...
    for (String oldCn : this.cn) {
      newCn.add(this.escapeChars(oldCn));
    }

    for (String oldOu : this.ou) {
      newOu.add(this.escapeChars(oldOu));
    }

    for (String oldDc : this.dc) {
      newDc.add(this.escapeChars(oldDc));
    }

    return new DN(newCn, newOu, newDc, this.o);
  }
  ...
  public List<DN> getAncestors() {
    return this.getAncestors(1, ADMINISTRATION);
  }
  ...
  private class DNIterator implements Iterator<DN> {
    private int cnPoint;
    private int ouPoint;
    private String oPoint = DN.this.o;
    ...
    @Override
    public DN next() {
      List<String> theCn = DN.this.cn;
      List<String> theOu = DN.this.ou;
      List<String> theDc = DN.this.dc;
      String theO = DN.this.o;

      if (DN.this.cn != null && this.cnPoint < DN.this.cn.size()) {
        theCn = DN.this.cn.subList(this.cnPoint++, DN.this.cn.size());
      } else if (DN.this.ou != null && this.ouPoint < DN.this.ou.size()) {
        theCn = new ArrayList<String>();
        theOu = DN.this.ou.subList(this.ouPoint++, DN.this.ou.size());
      } else if (this.oPoint != null) {
        theCn = new ArrayList<String>();
  ...
  public int hashCode() {
    ...
    result = prime * result + this.dc.hashCode();
    result = prime * result + this.o.hashCode();
    result = prime * result + this.ou.hashCode();
    return result;
  }
    ...
    if (this != obj) {
      if (obj == null) {
        ...
        if (this.getClass() != obj.getClass()) {
          ...
          equal &= this.cn.equals(other.cn);
          equal &= this.dc.equals(other.dc);
          equal &= this.o.equals(other.o);
          equal &= this.ou.equals(other.ou);
        }
      }
    }
    return equal;
