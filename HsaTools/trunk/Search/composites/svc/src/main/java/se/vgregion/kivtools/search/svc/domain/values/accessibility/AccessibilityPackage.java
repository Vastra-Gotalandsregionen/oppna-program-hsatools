...
public class AccessibilityPackage implements Serializable {
	...
	public AccessibilityPackage(Node accessibilityPackage) {
		...
		// Get criterias
		NodeList accessibilityPackageChildren = accessibilityPackage
				.getChildNodes();
		for (int i = 0; i < accessibilityPackageChildren.getLength(); i++) {
			// If node name is objectName, set name
			if (accessibilityPackageChildren.item(i).getNodeName().equals(
					"objectName")
					|| accessibilityPackageChildren.item(i).getNodeName()
							.equals("name"))
			...
			if (accessibilityPackageChildren.item(i).getNodeName().equals(
					"criteria")) {
				Criteria criteria = new Criteria(accessibilityPackageChildren
						.item(i));
