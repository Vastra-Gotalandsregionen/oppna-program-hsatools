...
public class GaussKrugerProjection implements CoordinateTransformerService {
	...
	private double A, B, C, D, beta1, beta2, beta3, beta4, e2, n, aHat, delta1,
			delta2, delta3, delta4, Astar, Bstar, Cstar, Dstar;
	...
	private void initialize() {
		...
		n = f / (2.0 - f);
		aHat = (a / (1.0 + n))
				* (1.0 + (0.25 * Math.pow(n, 2)) + ((1.0 / 64.0) * Math.pow(n,
						4)));
		A = e2;
	 ...
	 */
	private void calcGaussKrugerProjectionFromGeodeticToGrid(double lat,
			double lon) {
		// Compute the Conformal Latitude
		double phiStar = lat
				- (Math.sin(lat) * Math.cos(lat) * (A + B
						* Math.pow(Math.sin(lat), 2) + C
						* Math.pow(Math.sin(lat), 4) + D
						* Math.pow(Math.sin(lat), 6)));
		...
		rt90X = k0
				* aHat
				* (chi + beta1 * Math.sin(2.0 * chi) * Math.cosh(2.0 * eta)
						+ beta2 * Math.sin(4.0 * chi) * Math.cosh(4.0 * eta)
						+ beta3 * Math.sin(6.0 * chi) * Math.cosh(6.0 * eta) + beta4
						* Math.sin(8.0 * chi) * Math.cosh(8.0 * eta)) + FN;

		rt90Y = k0
				* aHat
				* (eta + beta1 * Math.cos(2.0 * chi) * Math.sinh(2.0 * eta)
						+ beta2 * Math.cos(4.0 * chi) * Math.sinh(4.0 * eta)
						+ beta3 * Math.cos(6.0 * chi) * Math.sinh(6.0 * eta) + beta4
	...
	private void calcGaussKrugerProjectionFromGridToGeodetic(int x, int y) {
		...
		double chiPrim = chi - delta1 * Math.sin(2 * chi) * Math.cosh(2 * eta)
				- delta2 * Math.sin(4 * chi) * Math.cosh(4 * eta) - delta3
				* Math.sin(6 * chi) * Math.cosh(6 * eta) - delta4
				* Math.sin(8 * chi) * Math.cosh(8 * eta);

		double etaPrim = eta - delta1 * Math.cos(2 * chi) * Math.sinh(2 * eta)
				- delta2 * Math.cos(4 * chi) * Math.sinh(4 * eta) - delta3
				* Math.cos(6 * chi) * Math.sinh(6 * eta) - delta4
		...
		lat = phiStar
				+ Math.sin(phiStar)
				* Math.cos(phiStar)
				* (Astar + Bstar * Math.pow(Math.sin(phiStar), 2) + Cstar
						* Math.pow(Math.sin(phiStar), 4) + Dstar
						* Math.pow(Math.sin(phiStar), 6));
	}
	...
	public int[] getRT90(double lat, double lon) {
		...
		int[] RT90Coordinates = {
				Integer.parseInt(String.valueOf(Math.round(rt90X))),
				Integer.parseInt(String.valueOf(Math.round(rt90Y))) };
	...
	public double[] getWGS84(int x, int y) {
		...
		this.calcGaussKrugerProjectionFromGridToGeodetic(Integer
				.parseInt(String.valueOf(Math.round(rt90X))), Integer
				.parseInt(String.valueOf(Math.round(rt90Y))));
