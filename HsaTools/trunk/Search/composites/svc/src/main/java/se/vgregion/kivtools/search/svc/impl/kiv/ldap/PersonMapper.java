...
public class PersonMapper implements ContextMapper {
	...
	public Person mapFromContext(Object ctx) {
		...
		List<String> hsaLanguageKnowledgeCode = getListFromArrayAttributes(dirContext.getStringAttributes(LDAPPersonAttributes.LANGUAGE_KNOWLEDGE_CODE
				.toString()));
		...
		// Anst�llningsperiod
		person.setEmploymentPeriod(TimePoint.parseFrom(getStringValue(LDAPPersonAttributes.HSA_START_DATE.toString()), "", TimeZone.getDefault()), TimePoint
				.parseFrom(getStringValue(LDAPPersonAttributes.HSA_END_DATE.toString()), "", TimeZone.getDefault()));
