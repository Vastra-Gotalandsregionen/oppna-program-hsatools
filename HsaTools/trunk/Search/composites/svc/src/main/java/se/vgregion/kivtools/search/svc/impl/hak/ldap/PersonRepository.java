...
public class PersonRepository {
	...
	public List<String> getAllPersonsVgrId() throws Exception {
		...
		try {
			...
			LDAPSearchResults searchResults = lc.search(KIV_SEARCH_BASE, LDAPConnection.SCOPE_SUB, searchFilter,
	...
	private Person searchPerson(String searchBase, int searchScope, String searchFilter) throws Exception {
		...
		try {
			lc = getLDAPConnection();
			result = extractSingleResult(lc.search(searchBase, searchScope, searchFilter, attributes, false, // return
																												// attributes
																												// and
																												// values
					constraints));
	...
	private SikSearchResultList<Person> searchPersons(String baseDn, String searchFilter, int searchScope, int maxResult)
			throws Exception {
		LDAPConnection lc = null;
		LDAPSearchConstraints constraints = new LDAPSearchConstraints();
		constraints.setMaxResults(0);
		...
		try {
			lc = getLDAPConnection();
			result = extractResult(lc.search(baseDn, searchScope, searchFilter, attributes, false, // return
																									// attributes
																									// and
																									// values
	...
	private SikSearchResultList<Person> extractResult(LDAPSearchResults searchResults, int maxResult)
	 ...
	 * 
	 * @param employments
	 *            - Map with employments that belongs to a person, persons cn
	 *            attribute is the key in the map.
	 * @param person
	 *            - person that extracted employment should belong to.
	 * @param personEntry
	 *            - LDAP entry to extract employment info from.
	 */
	...
	public static Employment extractEmployment(LDAPEntry personEntry) {
		...
		employment.setHsaStreetAddress(AddressHelper.convertToAddress(LdapORMHelper.getMultipleValues(personEntry
				.getAttribute("street"))));
		employment.setHsaInternalAddress(AddressHelper.convertToAddress(LdapORMHelper.getMultipleValues(personEntry
				.getAttribute("hsaInternalAddress"))));
		employment.setHsaPostalAddress(AddressHelper.convertToAddress(LdapORMHelper.getMultipleValues(personEntry
				.getAttribute("postalAddress"))));
		employment.setHsaSedfDeliveryAddress(AddressHelper.convertToAddress(LdapORMHelper.getMultipleValues(personEntry
				.getAttribute("hsaDeliveryAddress"))));
		employment.setHsaSedfInvoiceAddress(AddressHelper.convertToAddress(LdapORMHelper.getMultipleValues(personEntry
				.getAttribute("hsaInvoiceAddress"))));
		employment.setFacsimileTelephoneNumber(PhoneNumber.createPhoneNumber(LdapORMHelper.getSingleValue(personEntry
		...
		employment.setHsaSedfSwitchboardTelephoneNo(PhoneNumber.createPhoneNumber(LdapORMHelper
				.getSingleValue(personEntry.getAttribute("hsaSwitchboardNumber"))));
		employment.setName(LdapORMHelper.getSingleValue(personEntry.getAttribute("company")));
		employment.setHsaTelephoneNumbers(PhoneNumber.createPhoneNumberList(LdapORMHelper.getMultipleValues(personEntry
				.getAttribute("telephoneNumber"))));
		employment.setHsaPublicTelephoneNumber(PhoneNumber.createPhoneNumber(LdapORMHelper.getSingleValue(personEntry
				.getAttribute("hsaPublicTelephoneNumber"))));
		employment.setMobileTelephoneNumber(PhoneNumber.createPhoneNumber(LdapORMHelper.getSingleValue(personEntry
				.getAttribute("mobile"))));
		employment.setHsaInternalPagerNumber(PhoneNumber.createPhoneNumber(LdapORMHelper.getSingleValue(personEntry
		...
		employment.setHsaTextPhoneNumber(PhoneNumber.createPhoneNumber(LdapORMHelper.getSingleValue(personEntry
				.getAttribute("hsaTextPhoneNumber"))));
		employment.setModifyTimestamp(TimePoint.parseFrom(LdapORMHelper.getSingleValue(personEntry
				.getAttribute("whenChanged")), ""/* TODO Add pattern */, TimeZone.getDefault()));
		// employment.setModifyersName(LdapORMHelper.getSingleValue(personEntry.getAttribute("modifyersName")));
		employment.setHsaTelephoneTime(WeekdayTime.createWeekdayTimeList(LdapORMHelper.getMultipleValues(personEntry
				.getAttribute("hsaTelephoneTime"))));
		employment.setVgrStrukturPerson(DN.createDNFromString(LdapORMHelper.getSingleValue(personEntry
	...
	/**
	 * Add a search filter value to the filterList. e.g. searchField=givenName
	 * searchValue=hans result=(givenName=*hans*)
	 * 
	 * @throws Exception
	 */
	...
	private String createSearchFilterItem(String searchField, String searchValue) throws Exception {
		if (!Evaluator.isEmpty(searchValue)) {
			...
			if (isExactMatchFilter(searchValue)) {
				...
				// ")"); // exact match
				return "(" + searchField + "=" + searchValue.trim() + ")"; // exact
																			// match
			} else {
	...
	/**
	 * Special handling for givenName example: attribute1=givenName,
	 * attribute2=hsaNickName
	 * 
	 * e.g. searchValue = "hasse" return
	 * (|(givenName="hasse")(hsaNickName="hasse"))
	 * 
	 * e.g. searchValue = hasse return
	 * (|(givenName=*hasse*)(hsaNickName=*hasse*))
	 * 
	 * e.g. searchValue = hasse hans return
	 * (&(|(givenName=*hasse*)(hsaNickName=*
	 * hasse*))(|(givenName=*hans*)(hsaNickName=*hans*)))
	 * 
	 * @param filterList
	 * @param searchField
	 * @param searchValue
	 * @throws Exception
	 */
	private void addMultipleAttributes(List<String> filterList, String searchValue, String attribute1, String attribute2)
			throws Exception {
		if (!Evaluator.isEmpty(searchValue)) {
			searchValue = searchValue.trim();
			if (isExactMatchFilter(searchValue)) {
				...
				if (listSize == 0) {
					throw new Exception("Detected list.size==0 should never be possible! methodname=" + CLASS_NAME
							+ "addPersonGivenName()");
				}
				if (listSize == 1) {
					throw new Exception("Detected list.size==1 should never be possible! methodname=" + CLASS_NAME
							+ "addPersonGivenName()");
				}
