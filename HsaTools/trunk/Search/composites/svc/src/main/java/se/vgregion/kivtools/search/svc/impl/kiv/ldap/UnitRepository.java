...
public class UnitRepository {
	 ...
	 *            - show units for chosen HsaBussinessClassificationCodes.
	 * 
	...
	private boolean unitHasValidBusinessClassificationCode(List<Integer> showUnitsWithTheseHsaBussinessClassificationCodes,
	...
	public List<String> getAllUnitsHsaIdentity(List<Integer> showUnitsWithTheseHsaBussinessClassificationCodes) throws KivException {
		...
		AttributesMapper attributesMapper = new AttributesMapper() {
			...
			public Object mapFromAttributes(Attributes attributes) throws NamingException {
				return attributes.get(UnitLdapAttributes.HSA_IDENTITY);
			}
		};

		result = ldapTemplate.search("", searchFilter, SearchControls.SUBTREE_SCOPE, attributes, attributesMapper);
	...
	protected SikSearchResultList<Unit> searchUnits(String searchFilter, int searchScope, int maxResult, Comparator<Unit> sortOrder) throws KivException {
		...
		/*
		 * klar try { LDAPConnection lc = getLDAPConnection(); try { result = extractResult(lc.search(getSearchBase(), searchScope, searchFilter, attributes,
		 * false, constraints), maxResult, sortOrder); } finally { theConnectionPool.freeConnection(lc); } } catch (LDAPException e) { throw new
		 * KivException("An error occured in communication with the LDAP server. Message: " + e.getMessage()); }
		 */
		return result;
	}
	...
	private Unit searchUnit(String searchBase, int searchScope, String searchFilter) throws KivException {
		...
		/*
		 * try { LDAPConnection lc = getLDAPConnection(); try { result = extractSingleResult(lc.search(searchBase, searchScope, searchFilter, attributes, false,
		 * constraints)); } finally { theConnectionPool.freeConnection(lc); } } catch (LDAPException e) { throw new
		 * KivException("An error occured in communication with the LDAP server. Message: " + e.getMessage()); }
		 */
	...
	private String makeFunctionSearchStringFromUnitSearchString(String unitSearchString) {
		...
		functionSearchString = Formatter.replaceStringInString(functionSearchString, Constants.OBJECT_CLASS_UNIT_STANDARD,
				Constants.OBJECT_CLASS_FUNCTION_STANDARD);
		functionSearchString = Formatter.replaceStringInString(functionSearchString, Constants.LDAP_PROPERTY_UNIT_NAME + "=",
	...
	private String createUnitSearchFilter(SearchUnitCriterions searchUnitCriterions) {
		...
		if (!StringUtil.isEmpty(searchUnitCriterions.getAdministrationName())) {
			Filter orFilter = generateOrFilterFromList(CodeTableName.VGR_AO3_CODE, LDAPUnitAttributes.ADMINISTRATION, searchUnitCriterions
		...
		if (!StringUtil.isEmpty(searchUnitCriterions.getLocation())) {
			...
			OrFilter orPostalAddress = createAddressSearchFilter(filterList, "hsaPostalAddress", LdapParse.escapeLDAPSearchFilter(searchUnitCriterions
					.getLocation()));
			// Create or criteria
			OrFilter orStreetAddress = createAddressSearchFilter(filterList, "hsaStreetAddress", LdapParse.escapeLDAPSearchFilter(searchUnitCriterions
					.getLocation()));
	...
	public SikSearchResultList<Unit> getSubUnits(Unit parentUnit, int maxResult) throws KivException {
		...
		List<Unit> search = ldapTemplate.search(parentDn.toString(), "(objectClass=" + Constants.OBJECT_CLASS_UNIT_SPECIFIC + ")", LDAPConnection.SCOPE_SUB,
				unitMapper);
