...
public class UnitRepository {
     ...
     *            - show units for chosen HsaBusinessClassificationCodes.
     * 
    ...
    public SikSearchResultList<Unit> searchAdvancedUnits(Unit unit, int maxResult, Comparator<Unit> sortOrder,
            List<Integer> showUnitsWithTheseHsaBusinessClassificationCodes) throws KivException {
        ...
        SikSearchResultList<Unit> units = searchUnits(searchFilter, LDAPConnection.SCOPE_SUB, Integer.MAX_VALUE,
                sortOrder);
    ...
    protected void removeUnallowedUnits(SikSearchResultList<Unit> units,
        ...
        for (int j = units.size() - 1; j >= 0; j--) {
            ...
            boolean found = unitHasValidBusinessClassificationCode(
    ...
    private boolean unitHasValidBusinessClassificationCode(
            List<Integer> showUnitsWithTheseHsaBusinessClassificationCodes,
            List<String> businessClassificationCodes) {
        boolean found = false;
     ...
     * @throws KivException .
     */
    public SikSearchResultList<Unit> searchUnits(SearchUnitCriterions searchUnitCriterions, int maxResult)
            throws KivException {
     ...
     * @throws KivException .
     */
    public List<String> getAllUnitsHsaIdentity(List<Integer> showUnitsWithTheseHsaBusinessClassificationCodes)
            throws KivException {
        ...
        List<String> result = ldapTemplate.search(KIV_SEARCH_BASE, searchFilter, SearchControls.SUBTREE_SCOPE,
                attributes, new SingleAttributeMapper(UnitLdapAttributes.HSA_IDENTITY));
        return result;
    }
     ...
     * 
     * @param showUnitsWithTheseHsaBusinessClassificationCodes
     *            Only select units from search that has business codes from this list.
    ...
    public List<Unit> getAllUnits(List<Integer> showUnitsWithTheseHsaBusinessClassificationCodes) {
        ...
        List<Unit> result = ldapTemplate.search(KIV_SEARCH_BASE, searchFilter, SearchControls.SUBTREE_SCOPE,
                unitMapper);
        return result;
    }
    ...
    protected SikSearchResultList<Unit> searchUnits(String searchFilter, int searchScope, int maxResult,
            Comparator<Unit> sortOrder) throws KivException {
        LDAPSearchConstraints constraints = new LDAPSearchConstraints();
        constraints.setMaxResults(0);
        SikSearchResultList<Unit> result = null;
        List<Unit> search = ldapTemplate.search(KIV_SEARCH_BASE, searchFilter, SearchControls.SUBTREE_SCOPE,
                unitMapper);
    ...
    private Unit searchUnit(DistinguishedName searchBase, int searchScope, String searchFilter)
            throws KivException {
        LDAPSearchConstraints constraints = new LDAPSearchConstraints();
        constraints.setMaxResults(0);
    ...
    private SikSearchResultList<Unit> cleanAndSortResult(List<Unit> units, int maxResult,
    ...
    private String makeFunctionSearchStringFromUnitSearchString(String unitSearchString) {
        String functionSearchString = Formatter.replaceStringInString(unitSearchString,
                Constants.OBJECT_CLASS_UNIT_SPECIFIC, Constants.OBJECT_CLASS_FUNCTION_SPECIFIC);
        functionSearchString = Formatter.replaceStringInString(functionSearchString,
                Constants.OBJECT_CLASS_UNIT_STANDARD, Constants.OBJECT_CLASS_FUNCTION_STANDARD);
        functionSearchString = Formatter.replaceStringInString(functionSearchString,
                Constants.LDAP_PROPERTY_UNIT_NAME + "=", Constants.LDAP_PROPERTY_FUNCTION_NAME + "=");
    ...
    /**
     * Filter the units in a way that only units with a valid hsaBusinessClassificationCode or that should be
     * matched by an unfiltered health care type is included.
     * 
     * @param showUnitsWithTheseHsaBusinessClassificationCodes
     * @return
     * @throws KivException
     */
    private String makeShowUnitsWithTheseHsaBusinessClassificationCodesString(
            List<Integer> showUnitsWithTheseHsaBusinessClassificationCodes) {
         ...
         * showUnitsWithTheseHsaBusinessClassificationCodes into consideration.
         */
        ...
        logger.debug("Creating filter for hsaBusinessClassificationCode took: "
                + (endTimeMillis - startTimeMillis) + " milliseconds.");
        return orCriterias;
    }
    ...
    private String createUnitSearchFilter(SearchUnitCriterions searchUnitCriterions) {
        ...
        if (!StringUtil.isEmpty(searchUnitCriterions.getUnitName())) {
            ...
            orUnitName
                    .or(createSearchFilter(Constants.LDAP_PROPERTY_UNIT_NAME, searchUnitCriterions.getUnitName()));
            andFilter2.and(orUnitName);
        }
        if (!StringUtil.isEmpty(searchUnitCriterions.getAdministrationName())) {
            Filter orFilter = generateOrFilterFromList(CodeTableName.VGR_AO3_CODE,
                    LDAPUnitAttributes.ADMINISTRATION, searchUnitCriterions.getAdministrationName());
        ...
        if (!StringUtil.isEmpty(searchUnitCriterions.getBusinessClassificationName())) {
            Filter orFilter = generateOrFilterFromList(CodeTableName.HSA_BUSINESSCLASSIFICATION_CODE,
                    LDAPUnitAttributes.BUSINESS_CLASSIFICATION_CODE, searchUnitCriterions
                            .getBusinessClassificationName());
            andFilter2.and(orFilter);
        }
        if (!StringUtil.isEmpty(searchUnitCriterions.getCareTypeName())) {
            Filter orFilter = generateOrFilterFromList(CodeTableName.VGR_CARE_TYPE, LDAPUnitAttributes.CARE_TYPE,
        ...
        if (!StringUtil.isEmpty(searchUnitCriterions.getLocation())) {
            ...
            orMunicipalityName.or(createSearchFilter("hsaMunicipalityName", LdapParse
                    .escapeLDAPSearchFilter(searchUnitCriterions.getLocation())));
            // Create or criteria a bit special...
            OrFilter orPostalAddress = createAddressSearchFilter(filterList, "hsaPostalAddress", LdapParse
                    .escapeLDAPSearchFilter(searchUnitCriterions.getLocation()));
            // Create or criteria
            OrFilter orStreetAddress = createAddressSearchFilter(filterList, "hsaStreetAddress", LdapParse
                    .escapeLDAPSearchFilter(searchUnitCriterions.getLocation()));
    ...
    private Filter generateOrFilterFromList(CodeTableName codeTableName, LDAPUnitAttributes criterion,
    ...
    private String createAdvancedUnitSearchFilter(Unit unit) {
        ...
        // or criterias
        addSearchFilter(filterList, "hsaMunicipalityName", LdapParse.escapeLDAPSearchFilter(unit
                .getHsaMunicipalityName()));
        if (unit.getHsaMunicipalityCode() != null) {
            addSearchFilter(filterList, "hsaMunicipalityCode", LdapParse.escapeLDAPSearchFilter(unit
                    .getHsaMunicipalityCode()));
        }

        // a bit special...
        createAddressSearchFilter(filterList, "hsaPostalAddress", LdapParse.escapeLDAPSearchFilter(unit
                .getHsaMunicipalityName()));

        createAddressSearchFilter(filterList, "hsaStreetAddress", LdapParse.escapeLDAPSearchFilter(unit
                .getHsaMunicipalityName()));
        // (|(par1=value1)(par2=value2))
        String orCriterias = makeOr(filterList);
        ...
        if (unit.isVgrVardVal()) {
            addSearchFilter(filterList, "vgrVardval", LdapParse.escapeLDAPSearchFilter(LdapParse
                    .convertBooleanToString(unit.isVgrVardVal())));
        }
    ...
    /**
     * e.g. searchField=hsaPostalAddress searchValue="uddevalla" result= (|(hsaPostalAddress
     * =*uddevalla*$*$*$*$*$*)(hsaPostalAddress=*$*uddevalla*$*$*$*$*) (hsaPostalAddress
     * =*$*$*uddevalla*$*$*$*)(hsaPostalAddress=*$*$*$*uddevalla*$*$*) (hsaPostalAddress
    ...
    public SikSearchResultList<Unit> getSubUnits(Unit parentUnit, int maxResult) throws KivException {
        ...
        List<Unit> search = ldapTemplate.search(parentDn.toString(), "(objectClass="
                + Constants.OBJECT_CLASS_UNIT_SPECIFIC + ")", LDAPConnection.SCOPE_SUB, unitMapper);
