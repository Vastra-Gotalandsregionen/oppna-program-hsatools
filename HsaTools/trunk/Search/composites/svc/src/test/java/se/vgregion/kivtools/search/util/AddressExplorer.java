...
/**
 * Explore the available address information in the catalog. Helpful when the
 * address data is a mess and you need to know what could be parsed.
 * 
...
public class AddressExplorer {
	...
	public static void main(String[] args) throws Exception {
		/*
		 * Kick start our own bean factory. Reuse ordinary connection settings
		 * etc from HRIV.
		 */
		...
		ae.setSearchService((SearchService) factory
				.getBean("Search.SearchService"));
		ae.setTheConnectionPool((LdapConnectionPool) factory
				.getBean("Search.LdapConnectionPool"));
	...
	private void printStreetAddressReport() throws Exception {
		...
		for (Unit u : units) {
			...
			if (!Evaluator.isEmpty(u.getHsaStreetAddress().getStreet())
					&& !Evaluator.isEmpty(u.getHsaStreetAddress().getCity())) {
				out.print("OK! ");
				out.println(u.getHsaStreetAddress().getStreet().trim() + ", "
						+ u.getHsaStreetAddress().getZipCode() + " "
						+ u.getHsaStreetAddress().getCity() + "\n");
			} else {
				out.print("INVALID! ");
				invalidUnits++;

				/*
				 * Check it out directly by looking in the LDAP directory
				 * ourselves!
				 */
				String searchFilter = "(hsaIdentity=" + u.getHsaIdentity()
						+ ")";
				LDAPEntry entry = getEntry(searchFilter);
				out.println("hsaStreetAddress in catalog: "
						+ entry.getAttribute("hsaStreetAddress") + "\n");
			}
		}

		if (invalidUnits != 0) {
			int percentInvalid = (int) Math.round((double) invalidUnits
					/ (double) units.size() * 100);
			out.println("\n Bad ratio: " + percentInvalid + "% ("
					+ invalidUnits + " of " + units.size() + ").");
		} else {
	...
	private LDAPEntry getEntry(String searchFilter) throws Exception {
		...
		try {
			...
			return (lc.search(KIV_SEARCH_BASE, LDAPConnection.SCOPE_SUB,
					searchFilter, attributes, false, constraints).next());
		} finally {
